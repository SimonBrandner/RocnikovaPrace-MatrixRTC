...
není třeba o žádný backend starat, zároveň nehrozí, že při selhání backendu by
se uživatelé klienti spojit. Tento přístup má ale bohužel také mnoho potíží.

První potíž může být vytvoření samotného spojení dvou klientů -- v klasickém P2P
modelu obvykle dochází k tomu, že jeden uživatel volá druhého, to samé ale ve
full-mesh platit nemusí. Často totiž může již existovat skupinový hovor, když se
dva noví uživatelé pokusí připojit, a tak může být nejasné, který ze dvou
klientů bude volat druhého. Jedním řešením jsou \textit{glare resolution
...
identifikátorů klientů pro deterministické určení klienta, který budu toho
druhého volat.
...
je velký prostor pro nějakou chybu, hrozí poté tedy, že jeden z klientů budu
přijímat streamy jen od určité pod-skupiny klientů, zatímco jiný klient bude
přijímat stream od jiné pod-skupiny klientů, a tak dva uživatelé, kteří se
hovoru účastní můžou mít značně odlišný zážitek.
...
První z je, že streamy jsou mixované do jednoho streamu, který je pak pro
všechny klienty stejný, neumožňuje tedy uživatelům klientů, aby si vybrali,
kterému streamu budou věnovat pozornost atp.

Zároveň proces transkódování a mixování je poměrně náročný na výkon, tento
...
tento server přijímá streamy jednotlivých klientů a následně pod-skupinu těchto
...
jednotlivě. Tohle ale není tak velký problém, neboť SFU může posílat jen určitou
...
jsou to původní, poloviční a čtvrteční) a frameratech, SFU poté přeposílá
vhodnou verzi streamu. Vhodná verze se obvykle vybírá na základě několika
kritérií, prvním často je, v jakém rozlišení bude video zobrazeno na koncovém
...
nějaké bylo zvětšené. Dalším faktorem při výběru je poté spojení s jednotlivým
klienty a jeho kvalita.
