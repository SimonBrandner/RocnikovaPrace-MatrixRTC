...
Je spravován non-profitem jménem Matrix.org Foundation a jeho referenční
implementace používají permisivní licenci Apache.
...
Chce-li někdo navrhnout změnu protokolu, může napsat tzv. \textit{MSC} (Matrix
Spec Change) návrh. Jedná se o pull request\footnote{
	Pull request je způsob pro návrhu změn zdrojového kódu, diskuzi o nich a
	případné následné přijmutí těchto změn \cite{GitHub-PullRequests}.
}
...
specifikaci stará. Pokud se MSC dostane do stavu, kde nejsou vznášený připomínky
...
\subsection{Primitivy}

Matrix má několik základních primitivů, které si zde rozebereme.
...
\subsubsection{Místonsti a eventy}
...
Místnost obsahuje tzv. \textit{eventy} (\textit{events}), které jsou obvykle
produktem akce na straně klienta (např. poslání zprávy). Eventy se dělí na dva
druhy -- prvním druhem jsou \textit{message eventy}, jedná se např. o běžné
zprávy a reakce, druhým typem jsou \textit{state eventy}, které reprezentují
perzistentní informace jako je např. jméno místnosti, téma místnosti, členství v
místnosti atp.

Vzhledem k tomu, že state eventy můžou být měněny více uživateli, existuje tzv.
\textit{state resolution algorithm}, který umožňuje homeserverům se shodnout na
obsahu state eventů, pokud došlo k tomu, že homeservery nebyly nějakou dobu ve
spojení.

\subsubsection{To-device messages}

\textit{To-device messages} jsou dalším druhém eventů, který ale neexistuje v
místnosti, ale je přeposílán z jednoho zařízení do druhého. Aktuálně má hlavně
dvě využití -- šifrování (více v části \ref{matrix-encryption}) a skupinovém
...
} a klientů, z nichž je většina vyvíjena komunitou. Mezi ty známe patří např.
...
Microsoft adoptovali protokol Matrix, protokol umožňuje tzv. \textit{bridging}.
...
Protokol Matrix podporuje E2EE -- message eventy v místnostech mohou být
šifrované. Pro dosažení E2EE používá jak symetrické, tak asymetrické šifrování.
Jednotlivé message eventy jsou zašifrovány pomocí symetrického šifrování AES.
...
Aktuálně není možné šifrovat state eventy, ale existují MSCs, které by to
